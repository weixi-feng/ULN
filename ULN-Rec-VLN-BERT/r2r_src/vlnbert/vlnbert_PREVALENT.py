...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # NOTE: additional_layer
        # self.text_type_embeddings = nn.Embedding(3, config.hidden_size)
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        embeddings = words_embeddings + position_embeddings + token_type_embeddings # NOTE: add text type?
...
class VLNBert(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.lalayer_short = nn.ModuleList(
            [BertLayer(config) for _ in range(self.la_layers)]
        )
    ...
    def _resize_token_embeddings(self, new_num_tokens):
        ...
        # no need to worry about output embeddings
    ...
    def get_word_embeddings(self, input_ids, token_type_ids):
        embedding_output = self.embeddings(input_ids, position_ids=None, token_type_ids=token_type_ids)
        text_embeds = embedding_output
        return text_embeds

    def forward(self, mode, input_ids, token_type_ids=None,
        attention_mask=None, lang_mask=None, vis_mask=None, position_ids=None, head_mask=None, img_feats=None, return_attn=False, label=None):
        ...
        if mode == 'language':
            ...
            text_encoder = self.lalayer # default
            if label == 1:
                text_encoder = self.lalayer_short
            for layer_module in text_encoder:
            ...
            if not return_attn:
                ...
                return pooled_output, visual_action_scores, attended_language, attended_visual, language_attention_probs, visual_attention_probs, language_state_scores
