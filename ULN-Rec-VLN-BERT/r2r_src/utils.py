...
sys.path.append('/mnt/sshd/weixifeng/speaker_follower/build/')
import MatterSim
import string
import json
import time
...
def flat_list_of_list_vertical(list_of_list):
    n_element = len(list_of_list[0])
    n_list = len(list_of_list)
    new_list = []
    for i in range(n_element):
        for j in range(n_list):
            new_list.append(list_of_list[j][i])
    return new_list

def flat_list_of_list_horizontal(list_of_list):
    new_list = []
...
class Tokenizer(object):
    ...
    def __init__(self, vocab=None, encoding_length=20):
        ...
        self.add_word('<ANK>')
        self.add_word('<BOS>')
        assert self.vocab_size() == old+2
        print("OLD_VOCAB_SIZE", old)
        print("VOCAB_SIZE", self.vocab_size())

        self.set_bertlike_tokens()

    def set_bertlike_tokens(self):
        # NOTE: to be consistent with BertTokenizer
        self.cls_token = '<BOS>'
        self.cls_token_id = self.word_to_index[self.cls_token] # start of an instruction
        self.sep_token = '<EOS>'
        self.sep_token_id = self.word_to_index[self.sep_token] # end of an instruction, not sentence
        self.unk_token = '<UNK>'
        self.unk_token_id = self.word_to_index[self.unk_token]
        self.pad_token = '<PAD>'
        self.pad_token_id = self.word_to_index[self.pad_token]
        # self.ank_token = '<ANK>'
        # self.ank_token_id = self.word_to_index[self.ank_token]
    ...
    def shrink(self, inst, strict=True):
        """
        ...
        if not strict and end == 0:
            return list(inst[start:-1]) + [self.word_to_index['<EOS>']]
...
class BTokenizer(object):
    def __init__(self, args):
        ...
        self.encoding_length = args.maxInput
        self.split_regex = re.compile(r'(\W+)') # Split on any non-alphanumeric character
        
        # special_tokens = {'additional_special_tokens': ['[GOAL]', '[INSTR]']}
        # self.tokenizer.add_special_tokens(special_tokens)
        
        self.vocab_size = len(self.tokenizer)

        # self.goal_token = '[GOAL]'
        # self.goal_token_id = self.tokenizer.convert_tokens_to_ids(self.goal_token)
        # self.instr_token = '[INSTR]'
        # self.instr_token_id = self.tokenizer.convert_tokens_to_ids(self.instr_token)
    ...
    def insert_sep(self, encoding):
        offset = 0
        idx = [i for i, id in enumerate(encoding) if id==self.tokenizer.convert_tokens_to_ids(".")]
        for i in idx:
            if encoding[i+1+offset] != self.tokenizer.sep_token_id:
                encoding.insert(i+1+offset, self.tokenizer.sep_token_id)
                offset += 1
        return encoding

    def encode_sentence(self, sentence, seps=None, tokenized=False):
        txt = '[CLS] ' + sentence + ' [SEP]'

        if not tokenized:
            encoding = self.tokenizer.encode(txt)
        else:
            encoding = self.tokenizer.convert_tokens_to_ids(txt.split())

        # separate sentences
        encoding = self.insert_sep(encoding)
        ...
        # Align with Recurrent VLN Bert
        # if len(encoding) > self.encoding_length:
        #     encoding[self.encoding_length - 1] = self.tokenizer.sep_token_id

        return np.array(encoding[:self.encoding_length])

    # def merge_encode_sentences(self, sentences: list, tokenized=False):
    #     txt = []
    #     for i, sent in enumerate(sentences):
    #         if i == 0:
    #             txt.append('[CLS] [GOAL] ' + sent + ' [SEP]')    
    #         elif i == 1:
    #             txt.append('[INSTR] ' + sent + ' [SEP]')
    #         else:
    #             txt.append('[CLS] ' + sent + ' [SEP]')
    #     # txt = sentences # no cls between instr & goal

    #     txt = " ".join(txt)
    #     txt = txt.lstrip('[CLS]').rstrip('[SEP]').strip()
    #     encoding = self.encode_sentence(txt, tokenized=tokenized)
    #     return encoding
    ...
    def shrink(self, inst):
        """
        :param inst:    The id inst
        :return:  Remove the potential <CLS>, <SEP> and <PAD>
        """
        if len(inst) == 0:
            return inst
        if not isinstance(inst, np.ndarray):
            inst = np.array(inst)
        inst = np.delete(inst, np.where(inst==self.tokenizer.cls_token_id))
        inst = np.delete(inst, np.where(inst==self.tokenizer.sep_token_id))
        inst = np.delete(inst, np.where(inst==self.tokenizer.pad_token_id))
        return inst
